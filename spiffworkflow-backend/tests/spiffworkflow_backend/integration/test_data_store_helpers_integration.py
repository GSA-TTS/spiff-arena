...
class TestDynamicSchemaFieldsPipeline:
    ...
    def test_multiple_schema_fields_into_empty_store(self) -> None:
        ...
        # Raw value passed through as-is (no unwrapping)
        assert holder[1] == {
            "name": "naturalResourcesImpacts",
            "value": {"value": "some impact"},
        }
...
class TestExclusionsTextPipeline:
    ...
    def test_exclusions_text_with_wrapped_value(self) -> None:
        ...
        Values are passed through as-is — no unwrapping. The write subprocess
        stores the raw value in the holder.
        """
        task_data = {
            "exclusionsText": {
        ...
        # raw container is stored as-is (upsert keeps name+value from the entry)
        assert holder[0]["value"] == {
            "value": "CE text from form",
            "comment": {"comment": "reviewer note", "saved": True},
        }
...
class TestLupDecisionsPipeline:
    ...
    def test_lup_decisions_with_comment_through_transform(self) -> None:
        ...
        # Raw value is passed through as-is (no unwrapping)
        assert entry["value"] == {
            "value": "Specifically provided: Environmental Assessment",
            "comment": {"comment": "Reviewer approved", "saved": True},
        }
...
class TestNepareportIpacPipeline:
    ...
    def test_both_fields_present(self) -> None:
        ...
        holder = result["pid"]["model"]
        assert len(holder) == 2
        # Raw values passed through as-is (no unwrapping)
        assert holder[0] == {"name": "nepareport", "value": {"value": {"nepa": "data"}}}
        assert holder[1] == {"name": "ipacreport", "value": {"value": {"ipac": "data"}}}
...
class TestPipelineEdgeCases:
    ...
    def test_deeply_wrapped_values_passed_through(self) -> None:
        """Double-nested values are NOT unwrapped — passed through as-is."""
        task_data = {"field": {"value": {"value": "deeply nested actual value"}}}
        reader = _make_reader({})
        result = _pipeline(task_data, "field", reader, "pid", "model")

        holder = result["pid"]["model"]
        assert holder[0]["value"] == {"value": {"value": "deeply nested actual value"}}
